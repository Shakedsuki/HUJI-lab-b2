...
cv2.createTrackbar("G  S min", WIN,  80, 255, lambda x: None)
...
cv2.createTrackbar("R  S min", WIN, 120, 255, lambda x: None)
cv2.createTrackbar("R  S max", WIN, 255, 255, lambda x: None)
cv2.createTrackbar("R  V min", WIN,  80, 255, lambda x: None)
...
def process_frame(frame, circle_mask):
    ...
    # Red range 2: symmetric around 179/0 boundary
    ...
    # ── Overlay on original frame for left panel ──
    display = frame.copy()
    cv2.circle(display, PIVOT, MASK_RADIUS, (200, 200, 200), 1)  # mask boundary
    cv2.circle(display, PIVOT, 8, (0, 215, 255), -1)             # yellow pivot

    # Find and draw detected centroids
    ...
    # ── Convert grayscale masks to BGR for side-by-side display ──
    ...
    # Stack the three panels horizontally
    # All are 1280×720, so final image is 3840×720
    # Resize each to 1/3 width for display
    w = frame.shape[1] // 3
    h = frame.shape[0]
    panel = np.hstack([
...
def main():
    ...
    while True:
        ...
        # CAP_PROP_POS_FRAMES sets the next frame to read
        ...
        panel = process_frame(frame, circle_mask)
        cv2.imshow(WIN, panel)

        # Handle key presses
        # cv2.waitKey(30) waits 30ms and returns the key code
        # & 0xFF masks to the last 8 bits (cross-platform safety)
        key = cv2.waitKey(30) & 0xFF

        if key == ord('q') or key == 27:        # Q or ESC: quit
            break
        elif key == 83 or key == 3:             # RIGHT arrow: +10 frames
            frame_idx += 10
        elif key == 81 or key == 2:             # LEFT arrow: -10 frames
            frame_idx -= 10
        elif key == ord('d'):                   # D: +1 frame
            frame_idx += 1
        elif key == ord('a'):                   # A: -1 frame
            frame_idx -= 1
        elif key == ord(' '):                   # SPACE: jump to frame
            target = input("Jump to frame number: ")
            try:
                frame_idx = int(target)
            except ValueError:
                print("Invalid frame number")
        elif key == ord('s'):                   # S: save current values
            vals = {
                "GREEN_LOWER": (
                    cv2.getTrackbarPos("G  H min", WIN),
                    cv2.getTrackbarPos("G  S min", WIN),
                    cv2.getTrackbarPos("G  V min", WIN),
                ),
                "GREEN_UPPER": (
                    cv2.getTrackbarPos("G  H max", WIN),
                    cv2.getTrackbarPos("G  S max", WIN),
                    cv2.getTrackbarPos("G  V max", WIN),
                ),
                "RED_LOWER_1": (
                    cv2.getTrackbarPos("R  H min", WIN),
                    cv2.getTrackbarPos("R  S min", WIN),
                    cv2.getTrackbarPos("R  V min", WIN),
                ),
                "RED_UPPER_1": (
                    cv2.getTrackbarPos("R  H max", WIN),
