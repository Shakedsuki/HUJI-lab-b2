...
OUTPUT_CSV  = r"C:\dev\chaos\output\DSC_0136_trimmed_tracking.csv"
...
RED_LOWER_2 = np.array([165,  32,  72])   # mirror red range 2 to match
...
def find_centroid(mask):
    ...
    """
    # Find all separate white regions ("contours") in the mask
    # cv2.RETR_EXTERNAL  : only outermost contours (no nested)
    # cv2.CHAIN_APPROX_SIMPLE : compress contour to just corner points
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    if not contours:
        return None     # No blobs found at all

    # Keep only the largest contour by area — avoids noise blobs
    largest = max(contours, key=cv2.contourArea)

    if cv2.contourArea(largest) < MIN_BLOB_AREA:
        return None     # Largest blob is too small — probably noise

    # cv2.moments() computes geometric moments of the contour shape
    # The centroid is  x = M10/M00,  y = M01/M00
    # (M00 is the area, M10 and M01 are first-order spatial moments)
    M = cv2.moments(largest)

    if M["m00"] == 0:
        return None     # Degenerate contour (zero area after moments)
...
def compute_angle(point_from, point_to):
    ...
    Why atan2(dx, dy) and not atan2(dy, dx)?
    Image coordinates have y increasing DOWNWARD.
    We want angle from the downward vertical, so we swap axes:
      dx = horizontal displacement  →  sin component
      dy = vertical displacement    →  cos component (positive = down)

    Returns angle in degrees.
    """
    dx = point_to[0] - point_from[0]   # horizontal: right is positive
    dy = point_to[1] - point_from[1]   # vertical:   DOWN is positive (image coords)

    # atan2(dx, dy): angle from the +y axis (downward), measured clockwise
...
def build_circular_mask(frame_shape, center, radius):
    """
    Build a binary mask (same size as the frame) with a filled white circle.
    Pixels inside the circle = 255 (keep), outside = 0 (ignore).

    This restricts all color detection to within the pendulum's reachable area,
    preventing false positives from e.g. clothing or equipment at frame edges.
    """
    # np.zeros creates a black image of the same height/width as the frame
    mask = np.zeros(frame_shape[:2], dtype=np.uint8)

    # Draw a filled white circle (-1 = filled) at 'center' with given 'radius'
...
def main():
    # ── Open the video file ──
    # cv2.VideoCapture opens a video file (or camera if you pass an integer)
    ...
    # ── Build the circular mask once (it never changes) ──
    frame_shape = (height, width)
    circle_mask = build_circular_mask(frame_shape, PIVOT, MASK_RADIUS)

    # ── Set up debug video writer (optional) ──
    writer = None
    ...
    with open(OUTPUT_CSV, 'w', newline='') as csvfile:
        fieldnames = [
            ...
            'dropout'   # 1 if either marker was not detected, 0 if clean
        ]
        ...
        while True:
            ...
            # Frame is BGR (Blue-Green-Red) — OpenCV's default color order
            ret, frame = cap.read()
            ...
            # ── Convert BGR → HSV ──
            # HSV is much better for color isolation under varying lighting
            hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)

            # ── Build color detection masks ──
            # cv2.inRange: pixels within [lower, upper] → 255, outside → 0
            mask_green = cv2.inRange(hsv, GREEN_LOWER, GREEN_UPPER)

            mask_red_1 = cv2.inRange(hsv, RED_LOWER_1, RED_UPPER_1)
            mask_red_2 = cv2.inRange(hsv, RED_LOWER_2, RED_UPPER_2)
            # Combine both red ranges with OR — a pixel is "red" if in either range
            mask_red   = cv2.bitwise_or(mask_red_1, mask_red_2)

            # ── Apply circular mask — restrict to pendulum's reachable zone ──
            # bitwise_and: keeps only pixels that are BOTH in the color mask
            # AND inside the circle
            mask_green = cv2.bitwise_and(mask_green, circle_mask)
            mask_red   = cv2.bitwise_and(mask_red,   circle_mask)

            # ── Dilation — expand detected blobs to recover motion-blurred markers ──
            # A smeared marker passes the hue test but its pixels are too spread out
            # to form a blob above MIN_BLOB_AREA. Dilation fattens any surviving
            # pixels outward, merging them into a detectable contiguous region.
            ...
            # ── Compute angles (only if both markers detected) ──
            theta1 = None
            theta2 = None
            if not dropout:
                # θ₁: angle of arm 1 = vector from fixed pivot to green joint
                theta1 = compute_angle(PIVOT, green_pos)
                # θ₂: angle of arm 2 = vector from green joint to red tip
