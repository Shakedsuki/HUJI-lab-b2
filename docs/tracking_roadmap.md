...
_Generated 2026-05-01 23:51._ Re-run with `chaos roadmap`.
...
| **WARN** | 4 | tracking.csv exists, dropout/peak ω in WARN band |
| **FAIL** | 1 | tracking.csv exists but dropout > 10% or peak ω absurd |
| **HSV-ABORT** | 12 | ring_tracker bailed on the HSV adequacy probe — needs `chaos tune` |
...
| `th1_p001_th2_p093` | WARN | 2.14% | 4480 | 1 | 983 | 2026-05-01 | interp 1 |
...
- **12 clips need per-clip HSV tuning.** Run `chaos tune <stem>` then `chaos track <stem>` on each HSV-ABORT row above.
- **1 clips need a manual init/release pick.** Run `chaos next --stem <stem>` to drop into the picker.
- **4 clips landed in WARN.** Inspect `measurements/<stem>/verification.png`; if acceptable, mark verified; otherwise `chaos fix <stem>`.
